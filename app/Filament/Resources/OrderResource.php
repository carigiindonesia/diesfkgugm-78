        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('payment_method')
                    ...
                    ->badge()
                    ->formatStateUsing(fn (string $state) => PaymentMethod::from($state)->label())
                    ->color(fn (string $state) => PaymentMethod::from($state)->color())
                    ->toggleable(),
            ...
            ->filters([
                ...
                Tables\Filters\SelectFilter::make('payment_method')
                    ->label('Cara Pembayaran')
                    ->options(collect(PaymentMethod::cases())->mapWithKeys(fn ($p) => [$p->value => $p->label()])),
            ])
            ->actions([
            ...
            ->schema([
                ...
                Infolists\Components\Section::make('Pembayaran')
                    ...
                    ->schema([
                        ...
                        Infolists\Components\TextEntry::make('xendit_payment_method')
                            ->label('Metode Bayar Xendit')
                            ->visible(fn ($record) => $record->payment_method === 'payment_gateway'),
